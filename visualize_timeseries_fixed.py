...
def run_backtest_with_trades(symbol='BTCUSDT'):
    ...
    if trades:
        ...
        actual_win_rate = len(winning_trades) / len(trades) * 100
        results['win_rate'] = actual_win_rate / 100  # 转换为小数
...
def create_time_series_visualization(symbol='BTCUSDT'):
    ...
    win_rate = results.get('win_rate', 0) * 100  # 转换为百分比
    
    # 验证胜率计算
    winning_count = results.get('winning_trades', 0)
    losing_count = results.get('losing_trades', 0)
    
    fig.suptitle(f'{symbol} 交易时间序列分析\n总交易: {total_trades} 笔 | 总盈亏: ${total_pnl:.2f} | 胜率: {win_rate:.2f}% (盈利: {winning_count}, 亏损: {losing_count})',
    ...
    print(f"   胜率验证: {winning_count}/{total_trades} = {win_rate:.2f}%")
    plt.close()
