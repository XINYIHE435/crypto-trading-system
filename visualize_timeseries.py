...
def create_time_series_visualization(symbol='BTCUSDT'):
    ...
    gs = fig.add_gridspec(4, 1, hspace=0.15, height_ratios=[2, 1, 1, 1])
    ...
    ax1.set_title(f'{symbol} 价格走势与交易信号', fontsize=16, fontweight='bold')
    ...
    if trade_pnls:
        ...
        ax3.set_title('单笔交易盈亏分布', fontsize=14, fontweight='bold')
    ...
    fig.suptitle(f'{symbol} 交易时间序列分析\n总交易: {total_trades} 笔 | 总盈亏: ${total_pnl:.2f} | 胜率: {win_rate:.2f}%',
